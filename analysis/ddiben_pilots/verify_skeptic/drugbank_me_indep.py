...
def predictor(train_rows, known, k=10, alpha=1.0, scale=10.0):
    ...
    def dist(drug, role):
        ...
        if drug in known:
            ...
            s = TAN[drug, kn_arr]; top = np.argsort(-s, kind='stable')[:k]; nb = kn_arr[top]; w = s[top]
...
json.dump(res, open(f'out/phase3/skeptic_anchor/me_indep_{SPLIT}.json', 'w'), indent=1, default=str)
