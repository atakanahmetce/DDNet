...
for SPLIT in ['drugbank_random', 'drugbank_cluster']:
    ...
    for kname, S_ in KERNELS.items():
        ...
        Hn, Tn = knn(Hc), knn(Tc)
        res[kname] = {}
        for S in ['valid_S1', 'valid_S2']:
            te = np.loadtxt(d + f'{S}.txt', dtype=int); h, t, y = te[:, 0], te[:, 1], te[:, 2]
            ph = np.where(known[h][:, None], Hp[h], Hn[h]); pt = np.where(known[t][:, None], Tp[t], Tn[t])
            p = np.argmax(ph * pt / prior[None, :], 1)
            newd = np.where(known[h], t, h) if S.endswith('S1') else h
    ...
    res_all[SPLIT] = res
json.dump(res_all, open(OUT + 'kg_roleprior_valid.json', 'w'), indent=1)
